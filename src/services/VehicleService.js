...
export const VehicleService = {
  ...
  async search(searchCriteria = {}, page = 0, size = 10) {
    ...
    if (searchCriteria.bienSo) {
      params.append("bienSo", searchCriteria.bienSo);
    }
    if (searchCriteria.hangXe) {
      params.append("hangXe", searchCriteria.hangXe);
    }
    if (searchCriteria.namSanXuat) {
      params.append("namSanXuat", searchCriteria.namSanXuat);
    }
    if (searchCriteria.mauSac) {
      params.append("mauSac", searchCriteria.mauSac);
    }
  ...
  async update(id, data) {
    return await apiCall(`/api/xe/${id}`, {
      method: "PUT",
      data: data,
    });
  },
